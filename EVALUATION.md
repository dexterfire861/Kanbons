...
| ID | Criterion | Verify |
| --- | --- | --- |
| E1 | Nav shows Home, Customers, Products, Name matches, Stock, Incoming containers, Packing lists, Bills of lading, Warehouse check. Each listed route (and Home) loads with that heading. Home shows the Kanbons navy bar and logo. | `operator-assisted` — Playwright |
...
| E11 | `/metrics` is Prometheus text with `kanbons_database_up`, `kanbons_po_runs`, and `kanbons_po_extracted`. `/eval` returns JSON with those OCR field counts. Nav has no Metrics, Prometheus, or Grafana. | `operator-assisted` — Playwright |
| E12 | New packing slip can remove a line, blocks Confirm when a line qty is above Stock (shows Stock has N), and asks to read again when that PDF name was already saved. Bills of lading is in the nav. Leftover packing list shows `-2`. | `operator-assisted` — Playwright |
| E13 | Choosing a customer fills Ship to, and Bill to when that block is still empty. Ship to and Bill to can be different customers. The bill of lading page shows Ship to and the packing list number. A supplier PDF on Incoming containers is kept in Supplier documents. | `operator-assisted` — Playwright |
